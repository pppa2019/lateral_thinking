...
class Inference(STOGInferenceBase):
    ...
    @torch.no_grad()
    def parse_sents(self, sents, add_metadata=False, disable_progress=True):
        ...
        triples_list = [None]*len(sents)
        triples_final = [None]*len(sents)
        for snum in range(len(sents)):
            raw_graphs = graphs_generated[self._group_slice(snum)]
            for g in raw_graphs:
                if triples_list[snum] is None:
                    triples_list[snum] = PenmanDeSerializer(g).triples
                else:
                    triples_list[snum].extend(PenmanDeSerializer(g).triples)
            for triple in triples_list[snum]:
                if triples_final[snum] is None:
                    triples_final[snum] = [triple]
                elif triple not in triples_final[snum]:
                    triples_final[snum].append(triple)
        ...
        return graphs_final, triples_final
